...
void tc_t::circuit_t::_read() {
    ...
    m_path = std::unique_ptr<std::vector<std::unique_ptr<point_t>>>(new std::vector<std::unique_ptr<point_t>>());
    m_path->reserve(l_path);
    for (int i = 0; i < l_path; i++) {
        m_path->push_back(std::move(std::unique_ptr<point_t>(new point_t(m__io, this, m__root))));
    }
}
